...
6. **Merchant memory:** mỗi lần user sửa category sau khi AI gợi ý, app lưu mapping `keyword/merchant → category` vào bảng local. Lần sau gặp merchant đã biết, app override category bằng memory (ưu tiên hơn LLM) — không bao giờ bắt user sửa cùng một merchant hai lần, đồng thời giảm phụ thuộc và chi phí LLM. Memory áp dụng client-side sau parse để backbone server giữ stateless.
