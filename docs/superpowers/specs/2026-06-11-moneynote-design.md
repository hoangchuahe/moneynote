...
| State management | Riverpod | Modern, testable; xác nhận lại ở plan nếu anh quen Bloc hơn |
...
1. **Presentation (UI):** các màn hình — Home/Dashboard, Add Transaction (có ô smart input AI), Transactions list, Budgets, Reports, Settings. Widget chỉ đọc state từ Riverpod provider và gọi action.
...
|---|---|---|
| **1 — Core** | Scaffold monorepo; Flutter app: Drift schema + seed, CRUD giao dịch, danh mục, ví, dashboard cơ bản (tổng thu/chi tháng, list gần nhất) | **Chưa có AI, chưa cần backend.** App dùng được end-to-end offline |
